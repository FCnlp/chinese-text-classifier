...
def gen_data(filePathTrain):
    ...
    uniqueword_len = len(word2idx)
    # 初始化训练集和测试集
    return labelIds, reviewIds, uniqueword_len


labelIds, reviewIds, uniqueword_len = gen_data(
    filePathTrain=r"/Users/caowenli/Desktop/nlp/chineseGLUEdatasets.v0.0.1/inews/test.txt")
ont_hot_labelIds = keras.utils.to_categorical(labelIds, num_classes=3)
print(uniqueword_len)
reviewIds = sequence.pad_sequences(reviewIds, maxlen=maxlen)
print(reviewIds.shape)
print(len(labelIds))

textrnn = TextAttBiRNN(maxlen, uniqueword_len, embedding_dims, num_classes)
model = textrnn.get_model()
model.load_weights('TextAttBiRNN_model.h5')
